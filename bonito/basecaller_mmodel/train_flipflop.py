...
def prepare_random_batches(
        ...
        alphabet_info, filter_params, net_info = None, log = None,
    ...
    while total_sub_batches < target_sub_batches:
        ...
        if len(chunk_batch) < sub_batch_size:
            print(('* Warning: only {} chunks passed filters ' +
                       '(asked for {}).\n').format(
        ...
        for chunk in chunk_batch:
            ...
            #     chunk_labels, alphabet_info.ncan_base+1)
            chunk_seq = chunk_labels
...
def compute_filter_params(read_data):
    ...
    sampling_chunk_len = 2000 // 2
    sampling_chunk_len = (
        sampling_chunk_len // 5) * 5
...
def gen_batch(dir, limit = None, args = None):
    with mapped_signal_files.MappedSignalReader(dir) as msr:
        alphabet_info = msr.get_alphabet_information()
        # load list of signal_mapping.SignalMapping objects
        read_data = list(islice(msr.reads(None), limit))
    # res_info, logs = parse_init_args(args)
    # _, alphabet_info, mod_info = load_data(args, logs.main, res_info)
    # net_info, optim_info = load_network(
    #     args, alphabet_info, res_info, logs.main)
    filter_params = compute_filter_params(read_data)
